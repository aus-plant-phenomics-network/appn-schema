        ...
        if cls in property[0]:
            ...
            for r in property[1]:
                ...
                if cls == focus_class:
                    ...
                    this_class = f"[appn:{cls[1]}](appn_{cls[1]}.md)"
                else:
                    expanded_r = f"{prefixes[cls[0]]}{cls[1]}"
                    this_class = f"[{cls[0]}:{cls[1]}]({expanded_r})"
                if r[0] == "appn":
                    other_class = f"[appn:{r[1]}](appn_{r[1]}.md)"
                else:
            ...
            for r in property[0]:
                ...
                if r != cls or not reflexive:
                    ...
                    if cls == focus_class:
                        ...
                        this_class = f"[{cls[0]}:{cls[1]}](appn_{cls[1]}.md)"
                    else:
                        expanded_r = f"{prefixes[cls[0]]}{cls[1]}"
                        this_class = f"[{cls[0]}:{cls[1]}]({expanded_r})"
                    if r[0] == "appn":
...
def write_parents(
    ...
    if cls in inheritance:
        for parent in inheritance[cls]:
            if parent[0] == "appn":
                md.write(f"* [{prefixes[parent[0]]}{parent[1]}](appn_{parent[1]}.md)\n")
            else:
...
def write_children(
    ...
    if cls != focus_class:
        ...
        if cls[0] == "appn":
            md.write(f"* [{prefixes[cls[0]]}{cls[1]}](appn_{cls[1]}.md)\n")
        else:
