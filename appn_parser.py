...
class ExcelVocabularyParser:
    ...
    def __init__(self, dictionary: Dictionary, configuration: Configuration, node: Organisation):
        ...
        if self.node.id != "APPN":
            self.graph.bind(node.id.lower(), f"https://id.plantphenomics.org.au/{node.id}/")
    ...
    def process_sheet(self, excel_path: Path, sheet: str, sheet_class: Term) -> bool:
        ...
        for target_class, column_mappings in self.build_class_maps(df, sheet_class, embeddings).items():
            logging.debug(f"Mapping for: {target_class.curie}\n\n{''.join([f'  {m.column} --> {m.property} ({m.range_class.curie if m.range_class is not None else 'None'}, {m.primary_identifier})\n' for m in column_mappings])}")
            if not self.parse_instances_from_sheet(df, target_class, column_mappings):
                success = False

        return success

    def build_class_maps(self, df: pd.DataFrame, primary_class: Term, embeddings: list[Term]) -> dict[Term,list[ColumnMapping]]:

        class_maps : dict[str,list[ColumnMapping]] = {}

        class_maps[primary_class] = self.build_class_map(df, primary_class, "", [e.name.lower() for e in embeddings])
    ...
    def build_class_map(self, df: pd.DataFrame, target_class: Term, required_prefix: str, excluded_prefixes: list[str]) -> Optional[list[ColumnMapping]]:
        ...
        for p in dictionary.list_domain_properties_for_class(target_class.iri):
            if p.name not in properties:
                properties[p.name] = p
        for s in configuration.get_vocabulary_column_namespaces():
        ...
        for column in df.columns:
            ...
            if column_name.startswith(required_prefix) and not any([column_name.startswith(prefix) for prefix in excluded_prefixes]):
                if len(required_prefix) > 0 and column_name is not None and column_name.startswith(required_prefix):
                ...
                if column_property is None:
                    column_property = self.get_instance(rdf_property, f"https://id.plantphenomics.org.au/{node}/{self.lower_first(column_name)}")

                logging.info(
                    f"Column {column} recognised as {str(column_property)}"
                )

                column_mappings.append(ColumnMapping(column, column_property, related_class, column == name_column))

        return column_mappings if len(column_mappings) > 0 else None

    def parse_instances_from_sheet(self, df: pd.DataFrame, target_class: Term, column_mappings: list[ColumnMapping]) -> bool:
        ...
        if name_column is None:
            logging.error(f"Name column not specified for instances of target class {target_class.curie}")
            return False

        # Defer creating a concept scheme until we find at least one concept. 
        concept_scheme_term = self.concept_schemes[target_class] if target_class in self.concept_schemes else None
 
        for _, row in df.iterrows():
            if not pd.isnull(row[name_column]):
                name = row[name_column]
                term = self.get_instance(target_class, self.get_id(target_class.name, self.node.id, name, self.class_abbreviations), True)
                ...
                if term in self.instances:
                    ...
                    if concept_scheme_term is None:
                        concept_scheme_prefix = self.class_abbreviations['ConceptScheme'] if 'ConceptScheme' in self.class_abbreviations else 'conceptscheme'
                        concept_scheme_term = self.get_instance(skos_concept_scheme, f"https://id.plantphenomics.org.au/{self.node.id}/{concept_scheme_prefix}_{target_class.name}")
                        self.concept_schemes[target_class] = concept_scheme_term
                        for p in [schema_name, dc_title]:
                            self.graph.add((concept_scheme_term, p, Literal(target_class.name)))
                        for p in [schema_description, dc_description]:
                            value = Literal(f"Concept scheme including instances of the {target_class.curie} class from the APPN {node} node")
                            ...
                            if column_mapping.range_class is not None:
                                self.required_properties.append(
                                    URIRefTriple(
                                        ...
                                        property_term,
                                        URIRef(
                                            self.get_id(column_mapping.range_class.name, self.node.id, row[column_mapping.column], self.class_abbreviations)
                                        ),
                                    )
                                )
                            else:
                                ...
                                if property_term in self.property_expansions:
                                    for expansion_property in self.property_expansions[property_term]:
                                        self.graph.add((term, expansion_property, value_term))

        return success

    def get_instance(self, main_class: URIRef, id: str, is_concept: Optional[bool] = False) -> Term:
        term = URIRef(id)
    ...
    def list_explicit_classes(self, main_class: str|Term|URIRef, is_concept: Optional[bool] = False) -> list[Term]:
    ...
    def process_required_properties(self) -> bool:
        ...
        for required_property in self.required_properties:
            if required_property.object in self.instances:
                self.graph.add((required_property.subject,
    ...
    def get_id(self, class_name: str, node: str, name: str, abbreviations: dict[str, str]) -> str:
        clean_name = "".join(
    ...
    def sanitize_name(self, name: str) -> str:
        return "".join([w.title() for w in self.name_pattern.sub(" ", name).strip().split()])
